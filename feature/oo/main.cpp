...
public:
    ...
    *      5. 静态变量在多个对象之间是共享的，也就是说，即使实例化n多个对象，也只会使用一份内存来存储m_count，所有对象都是使用这份内存中的数据。
    */
    ...
    /* 析构函数
     *    1. 没有参数，不能有返回值，也不能被重载，一个类只能有一个析构函数
     *    2. 析构函数在释放变量的时候调用，用来实现一些删除对象前的清理操作
     */
    ~Person();
     ...
     *       1. 静态成员函数可以直接通过类来调用，也可以通过对象调用，编译器不会默认给函数传递参数this，因为他不需要对象的地址。
    ...
    /* const成员函数（常成员函数，类似常量，const成员函数中，不能对任何成员变量进行修改，因此一些只限制有读操作的method会声明为const成员函数）
